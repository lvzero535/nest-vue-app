    ...
    <template v-for="item in route.matched" :key="item.path">
      <Breadcrumb.Item>
        ...
        <template  v-if="item.children?.length" #overlay>
          <Menu @click="onMenuClick" :selected-keys="getSelectedKeys(item.children)">
            <Menu.Item v-for="childrenItem in item.children" :key="childrenItem.path">
...
const getSelectedKeys = (items: RouteRecordRaw[]) => {
  return items.filter(item => {
    return (route.path.includes(item.path) && item.children?.length) || route.path === item.path
  }).map(item => item.path);
}
